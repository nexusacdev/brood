...
import { AgentGenome, AgentRoundResult, SimulationRound, TokenData } from './types';
...
// Spawn threshold
const SPAWN_THRESHOLD = 150_000_000; // 0.15 SOL
const DEATH_THRESHOLD = 10_000_000;  // 0.01 SOL

async function runSimulation(rounds: number = 5) {
  ...
  for (const agent of AGENTS) {
    strategies.set(agent.name, new TradingStrategy(agent.genome));
  }

  // Get initial market data
  console.log('📊 Fetching market data...');
  let marketData = await fetchTrendingTokens(10);
  if (marketData.length === 0) {
    console.log('⚠️ API failed, using mock market data');
    marketData = generateMockMarket(10);
  }
  ...
  for (let round = 1; round <= rounds; round++) {
    ...
    for (const agent of AGENTS) {
      ...
      if (trades.length > 0) {
        ...
        console.log(`   Treasury: ${(agent.treasury / LAMPORTS_PER_SOL).toFixed(4)} SOL (${pnl >= 0 ? '+' : ''}${(pnl / LAMPORTS_PER_SOL).toFixed(4)})`);
  ...
  for (const agent of alive.sort((a, b) => b.treasury - a.treasury)) {
    const treasury = (agent.treasury / LAMPORTS_PER_SOL).toFixed(4);
    const canSpawn = agent.treasury >= SPAWN_THRESHOLD ? '✨' : '';
